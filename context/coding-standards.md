...
Boxers has one brand identity (not multiple selectable themes). Colors are defined as CSS custom properties and mapped to Tailwind tokens:

```css
@import "tailwindcss";

@theme {
  --color-primary: var(--brand-primary);         /* Orange — CTAs, headings, primary buttons */
  --color-secondary: var(--brand-secondary);      /* Dark green — accents, banners */
  --color-accent: var(--brand-accent);            /* Gold/yellow — highlights, stars */
  --color-surface: var(--brand-surface);          /* Cream/warm white — backgrounds */
  --color-surface-alt: var(--brand-surface-alt);  /* Light warm gray — card backgrounds */
  --color-text: var(--brand-text);                /* Dark charcoal — body text */
  --color-text-muted: var(--brand-text-muted);    /* Warm gray — secondary text */
...
- When porting Hound Around components, replace any theme-specific references with the Boxers brand tokens
- If the Hound Around codebase has a multi-theme toggle system, strip it — Boxers doesn't need it
...
│   │   │   ├── boarding/page.tsx
│   │   │   ├── grooming/page.tsx
│   │   │   ├── enrichment/page.tsx    # BEC — NEW (not in Hound Around)
│   │   │   ├── vet-clinic/page.tsx    # Meds & Fixits — NEW
│   │   │   └── training/page.tsx      # NEW
│   │   ├── pricing/page.tsx
│   │   ├── petcams/page.tsx
│   │   ├── our-staff/page.tsx         # NEW
│   │   └── employment/page.tsx        # NEW
│   ├── layout.tsx
...
│   └── dev/                      # Dev-only components (if any)
├── lib/
│   ├── sanity/
...
└── data/
    └── pricingData.ts            # Pricing calculator data (hardcoded, not in Sanity)
...
├── illustrations/                # Mascot logos, decorative elements
└── images/                       # Static images (logo variants, fallbacks)
```
...
- CSS custom properties: kebab-case (`--color-primary`, `--brand-surface`)
- Sanity document types: camelCase (`servicePage`, `pricingTier`)

## Styling

- Tailwind CSS for all styling
- Use semantic brand tokens for all colors and fonts
- No inline styles
